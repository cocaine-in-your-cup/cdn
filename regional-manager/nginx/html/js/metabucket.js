...
$(function() {
  ...
  var GCS_DOWNLOAD_URL = 'https://' + GCS_BUCKET + '.storage.googleapis.com';
  ...
  /**
   * Downloads a file from the specified URL when a button is clicked.
   */
  function download_contents() {
    ...
    anchor.setAttribute("href", contents_url);
  ...
  function fill_metadata(data) {
    ...
    dlstr += '</dl><br/>';
    dlstr += '<center><button id="btn-fetch-file" class="btn btn-secondary"style="margin: 1em;">' +
             'Fetch File</button>';
    dlstr += '<button id="btn-download-file" class="btn btn-primary" style="margin: 1em;">' +
             'Download File</button></center>';
    $('#file-props').html(dlstr);
    $('#btn-fetch-file').bind('click', fetch_contents);
    $('#btn-download-file').bind('click', download_contents);
